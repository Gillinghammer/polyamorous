...
from polly.models import Market, MarketOutcome
...
class PolymarketService:
    ...
    def _convert_market(self, market: dict) -> Market:
        ...
        # Use first tag as category (Polymarket's convention)
...
def filter_by_expiry_days(markets: List[Market], days: int) -> List[Market]:
    """Filter markets expiring within N days."""
    cutoff = datetime.now(tz=timezone.utc) + timedelta(days=days)
    return [m for m in markets if m.end_date <= cutoff]
